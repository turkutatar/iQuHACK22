...
def generate_QuantumCircuit_with_swap(input_str: str) -> QuantumCircuit:
    """
    Returns a QuantumCircuit given an input string containing the information about the data qubit error.
    ...
    # Swap gate between a pair of ancilla
    qc.swap(ancilla_qubits[1],ancilla_qubits[2])

    # Add the measurements into the circuit
    qc.measure(ancilla_qubits,creg)
    return qc

def execute_QuantumCircuit(qc, backend, shots=512):
    """
    ...
    ---------
        qc: QuantumCircuit
            QUantumCircuit object to be executed.
